...
### 5. 压缩后"重注入"(精髓)
`compactConversation`(compact.ts:393)压缩后**主动重建上下文**,不是丢完留摘要就完事:
...
- 顺序:boundary → summary → 保留尾 → 附件 → hookResults(`buildPostCompactMessages`, compact.ts:335)。
...
compactConversation()
  1. PreCompact hooks
  2. summary = 禁用工具的 fork agent 摘要(共享主缓存)
       └─ 爆窗重试: 按 API-round 丢最老组 ≤3
  3. 边界: groupMessagesByApiRound
  4. 重建 = boundary + summary + 保留尾 + 重注入附件(文件/plan/skill/goal/工具)
...
**4.4 推荐路线(优先级排序)**:
1. **(P0)工作视图 vs produced 分离** — loop 内维护"可能被压的发给模型的视图"与"完整 produced(用于返回/持久化)"分离。正确性地基。
2. **(P0)按 turn/round 切分 + ensureToolResultPairing** — 修 `Compress` 切断 tool 配对的硬伤;配对修复独立成函数,发送前兜底。
3. **(P0)LLM 压缩器** — 禁用工具的 sub-call,复用 provider adapter + 精简版结构化摘要 prompt。
