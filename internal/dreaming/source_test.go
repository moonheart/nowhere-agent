...
func TestWorkerIncrementalOverStores(t *testing.T) {
	...
	// extract → 1 fact; compress → a summary; consolidate folds both in.
	...
	if llm.calls != 3 {
		t.Errorf("llm calls = %d want 3 (extract+compress+consolidate; no work on the no-op pass)", llm.calls)
	}
