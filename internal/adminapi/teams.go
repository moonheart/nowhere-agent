...
func (h *Handler) listKeys(w http.ResponseWriter, r *http.Request) {
	if h.keys == nil {
		writeError(w, http.StatusServiceUnavailable, "provider keys unavailable")
		return
	}
	keys, err := h.keys.ListTeamKeys(r.Context(), r.PathValue("id"))
	if err != nil {
		writeServiceError(w, err)
		return
	}
	out := make([]teamKeyDTO, 0, len(keys))
	for _, k := range keys {
		out = append(out, teamKeyDTO{Provider: k.Provider, Masked: k.Masked, CreatedAt: k.CreatedAt, UpdatedAt: k.UpdatedAt})
	}
...
func (h *Handler) putKey(w http.ResponseWriter, r *http.Request) {
	...
	if !supportedProvider(provider) {
		writeError(w, http.StatusBadRequest, "provider must be one of: "+strings.Join(supportedProviders, ", "))
		return
	}
	k, err := h.keys.UpsertTeamKey(r.Context(), r.PathValue("id"), provider, key)
	if err != nil {
		writeServiceError(w, err)
		return
	}
...
func (h *Handler) deleteKey(w http.ResponseWriter, r *http.Request) {
	...
	deleted, err := h.keys.DeleteTeamKey(r.Context(), r.PathValue("id"), r.PathValue("provider"))
	if err != nil {
		writeServiceError(w, err)
		return
	}
...
// supportedProviders are the vendors the server can actually call. Rejecting
// anything else stops a team from storing a key under a typo'd provider name
// and then wondering why it never takes effect.
